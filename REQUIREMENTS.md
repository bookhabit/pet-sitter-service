...
- 나는 구인공고를 기준에 따라 필터링해서 페이지 단위로 조회할 수 있다 (동물 유형, 크기, 활동 유형 등)
...
#### 5.2.1 Job 모델 관련
- **Create**: 나는 펫시터에 한 마리 이상의 여러 유형의 반려동물 정보를 포함하는 구인 공고를 올릴 수 있다
...
- **List with Filtering**: 나는 구인공고를 기준에 따라 필터링해서 페이지 단위로 조회할 수 있다
  - 필터링 기준: 동물 유형(animalType), 크기(size), 활동 유형(activity), 날짜 범위 등
...
| 역할 | 주요 기능 |
|------|----------|
| **반려동물 주인** | 여러 반려동물 정보를 포함한 구인공고 등록/수정/삭제, 지원자 확인 및 승인, 계정 관리 |
| **돌봄 도우미** | 필터링/페이징/검색 기능을 활용한 구인공고 조회, 지원, 계정 관리 |
...
### 8.2 구인공고 관련
- `POST /jobs` - 구인공고 등록 (여러 반려동물 정보 포함 가능)
- `GET /jobs` - 구인공고 목록 조회
  - 쿼리 파라미터:
    - `page`: 페이지 번호 (기본값: 1)
...
- **반려동물 돌봄 도우미는 구인공고를 기준에 따라 필터링해서 페이지 단위로 조회할 수 있다**
  - 동물 유형, 크기, 활동 유형, 날짜 범위 등 다양한 기준으로 필터링 가능
  - 페이지 단위로 결과를 조회하여 대량의 데이터를 효율적으로 처리
- **반려동물 주인은 한 마리 이상의 여러 유형의 반려동물 정보를 포함하는 구인 공고를 등록할 수 있다**
  - 하나의 구인공고에 여러 반려동물 정보를 포함하여 등록 가능
  - 다양한 동물 유형(강아지, 고양이, 새 등)을 하나의 공고에 포함 가능
